...
class ImageAnalysisCore:
    ...
    def calculate_visual_edge_bounds(self, img_rgba: np.ndarray, threshold: float = 0.1) -> Dict[str, int]:
        """
        Calculates the "Visual Edge" bounds based on a strict opacity threshold.
        Useful for determining where the "haze" ends and the "solid" content begins.
        """
        return self.calculate_visible_bbox(img_rgba, threshold=int(255 * threshold))

    def calculate_anchor_points(self, img_rgba: np.ndarray, bounds: Dict[str, int]) -> Dict[str, Dict[str, int]]:
        """
        Finds the "Anchor Points" of the internal void.
        These are the absolute extremum points of the usable void space.
        
        For example:
        - "left": The right-most pixel of the left wall (the start of the void).
        - "right": The left-most pixel of the right wall.
        
        This is critical for organic shapes where the "LIR" might be conservative,
        but the user wants to know exactly where the content CAN physically touch.
        """
        alpha = img_rgba[:, :, 3]
        h, w = alpha.shape
        
        # Define "Void" as significantly transparent (consistent with LIR)
        void_threshold = 13
        void_mask = (alpha < void_threshold)
        
        # Constrain to layout bounds to avoid background noise
        bx, by = bounds["x"], bounds["y"]
        bw, bh = bounds["width"], bounds["height"]
        
        # Crop to bounds to optimize search
        search_region = void_mask[by:by+bh, bx:bx+bw]
        if search_region.size == 0:
             return {"left": {"x": bx, "y": by}, "right": {"x": bx + bw, "y": by}}

        rows, cols = search_region.shape
        
        # Find Left Anchor (First void pixel from left, averaged over central rows)
        # simplistic strategy: Find the min_x of the void in the central band
        center_band_y_start, center_band_y_end = int(rows * 0.4), int(rows * 0.6)
        center_band = search_region[center_band_y_start:center_band_y_end, :]
        
        # Default anchors
        left_anchor = {"x": bx, "y": by + bh // 2}
        right_anchor = {"x": bx + bw, "y": by + bh // 2}
        top_anchor = {"x": bx + bw // 2, "y": by}
        bottom_anchor = {"x": bx + bw // 2, "y": by + bh}

        if np.any(center_band):
            # Left Anchor: The left-most void pixel in the center band
            # actually, for a container, "Anchor Left" is usually the Right-most pixel of the Left Wall.
            # But the user compliance asking for "furthest vertex of an internal void".
            # Let's interpret "Left Anchor" as the left-most point of the Valid Void.
            
            # Find all void pixels
            void_y, void_x = np.where(center_band)
            if len(void_x) > 0:
                min_x = np.min(void_x)
                max_x = np.max(void_x)
                
                left_anchor = {"x": int(bx + min_x), "y": int(by + bh // 2)}
                right_anchor = {"x": int(bx + max_x), "y": int(by + bh // 2)}

        # Find Top/Bottom Anchors (similar central band logic vertical)
        center_band_x_start, center_band_x_end = int(cols * 0.4), int(cols * 0.6)
        center_band_v = search_region[:, center_band_x_start:center_band_x_end]
        
        if np.any(center_band_v):
            void_y, void_x = np.where(center_band_v)
            if len(void_y) > 0:
                min_y = np.min(void_y)
                max_y = np.max(void_y)
                
                top_anchor = {"x": int(bx + bw // 2), "y": int(by + min_y)}
                bottom_anchor = {"x": int(bx + bw // 2), "y": int(by + max_y)}
    ...
    def calculate_inner_radii(
        ...
        """
        Calculates the radius of the inner void's corners.
        STRICTLY for geometric shapes (rounded_rectangle). Returns None for organic.
        """
        if shape_type not in ["rounded_rectangle", "geometric"]:
            return None
            
        alpha = img_rgba[:, :, 3]
        bx, by = layout_bounds["x"], layout_bounds["y"]
        bw, bh = layout_bounds["width"], layout_bounds["height"]
        
        # Extract the void region (inverse of alpha) within layout bounds
        crop = alpha[by:by+bh, bx:bx+bw]
        if crop.size == 0:
            return None
            
        # Void Mask: White = Void
        void_mask = (crop < 13).astype(np.uint8) * 255
        
        # Find contours of the void
        contours, _ = cv2.findContours(void_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if not contours:
            return None
            
        main_contour = max(contours, key=cv2.contourArea)
        
        # Robustness Check: Is the VOID actually geometric?
        # If the outer frame is a rectangle (e.g. screen border), the void might still be irregular (vine).
        # We must classify the void shape itself to avoid hallucinating a radius for a vine.
        
        area = cv2.contourArea(main_contour)
        if area == 0:
            return None
            
        # Robustness Check 2: Solidity (Convexity)
        # Geometric voids (Pills, Rounded Rects) are convex.
        # Organic voids with intrusions (Vines) are concave.
        hull = cv2.convexHull(main_contour)
        hull_area = cv2.contourArea(hull)
        solidity = area / hull_area if hull_area > 0 else 0
        
        # If void is not solid enough (> 0.95), it has intrusions -> Organic
        if solidity < 0.95:
             return None

        # Estimate radius using the ACTUAL void bounds, not the container bounds
        vx, vy, vw, vh = cv2.boundingRect(main_contour)
        return self._estimate_corner_radius(main_contour, vw, vh)

    def calculate_arc_centers(
        self, 
        img_rgba: np.ndarray,
        layout_bounds: Dict[str, int], 
        ...
        """
        Calculates exact (x, y) coordinates for the centers of corner arcs.
        Strictly for geometric shapes where a reliable radius was found.
        """
        if not inner_radius or shape_type not in ["rounded_rectangle", "geometric", "circle"]:
            return None
        ...
        # Re-detect void bounds to get exact offsets
        crop = alpha[by:by+bh, bx:bx+bw]
        if crop.size == 0:
            return None
        ...
        # Calculate absolute void origin
        abs_vx = bx + vx
        abs_vy = by + vy
        
        r = inner_radius
        
        # Assuming symmetric rounded rect void
        return {
            "top_left": {"x": abs_vx + r, "y": abs_vy + r},
            "top_right": {"x": abs_vx + vw - r, "y": abs_vy + r},
            "bottom_left": {"x": abs_vx + r, "y": abs_vy + vh - r},
            "bottom_right": {"x": abs_vx + vw - r, "y": abs_vy + vh - r}
        }
    ...
    def calculate_void_density(self, img_rgba: np.ndarray, bounds: Dict[str, int]) -> float:
        """
        Calculates the percentage of pixels within the bounds that are 'void' (transparent).
        Used to detect if a container is mostly empty (like a pill) despite a small LIR.
        """
        alpha = img_rgba[:, :, 3]
        h, w = alpha.shape
        ...
        # Crop to bounds
        # Handle edge cases where bounds might be off-canvas (clamped)
        ...
        # Void = Alpha < 100 (Consistent with our "squint" threshold)
        void_pixels = np.sum(crop < 100)
    ...
    def check_symmetry(self, img_rgba: np.ndarray) -> Dict[str, float]:
        """
        Calculates symmetry scores for vertical (flip LR) and horizontal (flip UD) axes.
        Returns IoU (Intersection over Union) of the mask with its flipped version.
        score 1.0 = perfect symmetry.
        """
        alpha = img_rgba[:, :, 3]
        # Binary mask (threshold > 10 to ignore invisible pixels)
        ...
        intersection_lr = np.logical_and(mask, flipped_lr)
        union_lr = np.logical_or(mask, flipped_lr)
        score_v = np.sum(intersection_lr) / np.sum(union_lr) if np.sum(union_lr) > 0 else 0
        
        # Horizontal Symmetry (Flip Up-Down)
        flipped_ud = np.flipud(mask)
        intersection_ud = np.logical_and(mask, flipped_ud)
        union_ud = np.logical_or(mask, flipped_ud)
        score_h = np.sum(intersection_ud) / np.sum(union_ud) if np.sum(union_ud) > 0 else 0
        
        return {
            "vertical": score_v,   # Left-Right symmetry
            "horizontal": score_h  # Top-Bottom symmetry
        }
